package com.gamicarts.hipsterbirdfree.game.graphics;
...
import com.gamicarts.hipsterbirdfree.sprite.component.BirdComponentSet;
import com.gamicarts.hipsterbirdfree.sprite.data.Accessory;
import com.gamicarts.hipsterbirdfree.sprite.data.AccessoryType;
...
public class Shop {
    ...
    public static int shopPoints = 2;
            ...
            if (functionButton.selected(currentState, currentX, currentY)) {
                if (functionButton == FunctionButton.PAGE_LEFT) {
                    ...
                    if (selectedAccessory != null
                            ...
                            && selectedAccessory.getCost() < 3) { //Can't buy shop items 3 or more cost for the free version.
                        Utility.playSound(R.raw.button);
                        shopPoints -= selectedAccessory.getCost();
    ...
    private static void drawPage(Accessory[] accessories, int page, Canvas canvas, Paint paint) {
        ...
        paint.setTypeface(Typeface.create(HipsterBirdFree.HOBO, Typeface.NORMAL));
        ...
        for (int i=((page-1) * 8); i < (page * 8); i++) {
            ...
            if (i < accessories.length) {
                ...
                //Showing buy full version
                if (accessories[i].getCost() >= 3) {
                    paint.setStyle(Paint.Style.FILL);
                    paint.setColor(Color.argb(100, 0, 0, 0));
                    canvas.drawRect(surrounding, paint);

                    paint.setStyle(Paint.Style.FILL);
                    paint.setColor(Color.WHITE);

                    paint.getTextBounds("to unlock item", 0, "to unlock item".length(), stringBounds);
                    stringPoint = Utility.getStringBottomCenter(surrounding, stringBounds);
                    canvas.drawText("to unlock item", stringPoint.x, stringPoint.y, paint);

                    final float bfvy = stringPoint.y - Utility.getYRatio(20);
                    paint.getTextBounds("Buy full version", 0, "Buy full version".length(), stringBounds);
                    stringPoint = Utility.getStringBottomCenter(surrounding, stringBounds);
                    stringPoint.y = (int) bfvy;
                    canvas.drawText("Buy full version", stringPoint.x, stringPoint.y, paint);
                } else if (!accessories[i].unlocked
        ...
        for (BirdData bd : BirdData.values()) {
            if (bd.getID() == selectedAccessory.getID()) {
                if (bd != hipsterBird.getBirdData()) {
                    ...
                    bd.allocateComponentImages(HipsterBirdFree.RESOURCES, true);
                    ...
                    if (BirdData.TEMP.getBirdComponentSet() != hipsterBird.getBirdData().getBirdComponentSet()) { //Just in case they spam click the default.
                        BirdData.TEMP.allocateComponentImages(HipsterBirdFree.RESOURCES, true);
                    }
